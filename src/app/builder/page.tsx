...
import { Layers, Plus, ExternalLink, Edit, Trash2, Table, Zap, FileText, Share2, Copy, MoreHorizontal, LogOut, Search, Upload, LayoutTemplate, Sparkles, Users, Globe, Lock, Grid2X2, Home, BookOpen } from "lucide-react";
...
function BuilderHome() {
  ...
  const loadApps = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [list, tpls] = await Promise.all([storageService.getApps(user.email, true), storageService.listTemplates()]);
      setApps(list);
      setTemplates(tpls);
    } catch (err) {
      console.error(err);
  ...
  const duplicate = async (app: AppDefinition) => {
    const copy: AppDefinition = { ...JSON.parse(JSON.stringify(app)), id: generateId("app"), name: `${app.name} Copy`, linkName: `${app.linkName}_copy_${Date.now().toString(36).slice(-4)}`, members: [], memberEmails: [], publishedVersion: undefined, publishedAt: undefined, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
      ...
      <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white shadow-2xs"><Layers className="w-5 h-5" /></div>
            <div><span className="text-sm font-bold text-slate-900 tracking-tight">YourBuilder</span><span className="block text-[10px] text-slate-400 font-medium -mt-0.5">Low-code platform · Owner console</span></div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button variant="primary" size="sm" onClick={() => setCreateOpen(true)} icon={<Plus className="w-4 h-4" />}>Create Application</Button>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-10 space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Every app is built from scratch in the builder and stored in Firestore. Share each app with members by designation.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white p-14 rounded-2xl border-2 border-dashed border-slate-300 text-center space-y-4">
            ...
            <div><h2 className="text-base font-semibold text-slate-800">{apps.length === 0 ? "No applications yet" : "No matches"}</h2><p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Create your first application — forms, reports, workflows and dashboards are all designed in the builder.</p></div>
            <Button size="sm" onClick={() => setCreateOpen(true)} icon={<Plus className="w-3.5 h-3.5" />}>Create Application</Button>
          </div>
        ) : (
  ...
  const create = async () => {
    ...
    try {
      ...
      if (tab === "template" || tab === "duplicate") {
        ...
        for (const k of ["id", "name", "linkName", "members", "memberEmails", "ownerEmail", "publishedVersion", "createdAt", "isTemplate"]) delete rest[k];
        base = rest;
      } else if (tab === "import") {
